...
import { Package, Clock, Armchair, Check, X } from "lucide-react";

// ─── Types ───
type OrderItem = {
  id: number; // This acts as the Qty/Index badge in the UI
  name: string;
  price: number;
};
...
const seed: Order[] = [
  ...
  {
    id: "2",
    number: 1, // Keeping as #1 to match image visual, usually this increments
    time: "Just now",
    table: "Table 001",
    total: 497,
    items: [
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 min-h-[600px]">
      ...
      <div className="flex flex-col gap-6">
        ...
        {orders.map((order) => (
          <div
            key={order.id}
            className="bg-[#F3F4F6] rounded-2xl p-4 sm:p-5"
          >
            {/* ─── Order Header ─── */}
